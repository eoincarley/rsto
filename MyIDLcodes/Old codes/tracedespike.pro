...
;
;-
function tracedespike,image,nspikes=nspikes,imap=imap,ri=ri,$
...
if keyword_set(statistics) then begin
  print,'Eliminating outliers beyond ',fix(sigma+.5),$
    ' sigma using A=',ampl
  map(3,3)=((imout-imring))(3:nx,3:ny) 
  imap=where(map^2*(ampl/sigma^2) gt imout)
endif else begin
print,'Eliminating positive outliers deviating more than',fix(threshold*100),$ 
    ' percent.'
...
print,'Corrected ',n_elements(imap),' radiation hits and bad pixels'
